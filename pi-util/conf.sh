...
PATH="$TOOLS/bin:$PATH" \
  ...
  UIC="`which uic` -qt=5" \
  RCC="`which rcc` -qt=5" \
  ./configure --host=$A --enable-mmal-avcodec --disable-vdpau --disable-libva --enable-debug --disable-lua --disable-chromecast --disable-wayland --enable-gles2 --disable-opencv
