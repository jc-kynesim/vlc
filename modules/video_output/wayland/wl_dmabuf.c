...
static shared_count_t *
shared_count_new()
{
    shared_count_t * const sc = malloc(sizeof(*sc));
...
static int
shared_count_ref(shared_count_t * const sc)
{
    return atomic_fetch_add(&sc->n, 1) + 1;
}

static void
shared_count_unref(shared_count_t ** const ppsc)
{
    shared_count_t * const sc = *ppsc;
    if (sc == NULL)
        return;
    *ppsc = NULL;
    if (atomic_fetch_sub(&sc->n, 1) != 0)
        return;
    free(sc);
}

static int
shared_count_val(const shared_count_t * const sc)
{
    return atomic_load(&sc->n);
}
...
vdre_free(video_dmabuf_release_env_t * const vdre)
{
    ...
    eq_unref(&vdre->eq);
    shared_count_unref(&vdre->in_flight);
#if CHECK_VDRE_COUNTS
...
static void
vdre_in_flight_ref(video_dmabuf_release_env_t * const vdre, shared_count_t * const sc)
{
    shared_count_ref(sc);
    vdre->in_flight = sc;
}
...
subpic_ent_attach(subplane_t * const plane, subpic_ent_t * const spe, eq_env_t * const eq)
{
    ...
    {
        vdre_in_flight_ref(spe->vdre, plane->in_flight);
...
static void
spe_free(subpic_ent_t * const spe)
{
        ...
        const subpicture_region_t * const sreg)
{
    ...
    if ((spe->pt = polltask_new_timer(sys->speq, spe_convert_cb, spe)) == NULL) {
        spe_free(spe);
        return NULL;
    }
...
plane_destroy(subplane_t * const spl)
{
    ...
    shared_count_unref(&spl->in_flight);
}
             ...
             const bool sync)
{
    ...
    if ((plane->surface = wl_compositor_create_surface(video_compositor(sys))) == NULL ||
        ...
        (plane->in_flight = shared_count_new()) == NULL)
        return VLC_EGENERIC;
#if HAVE_WAYLAND_COLOR_REPRESENTATION
    if (sys->bound.color_representation_manager_v1 != NULL)
...
//    msg_Info(vd, "<<< %s: Surface: %p", __func__, sys->embed->handle.wl);
    ...
    for (unsigned int i = PLANE_VID; i != PLANE_SUB + MAX_SUBPICS; ++i)
    {
        ...
        {
            if (shared_count_val(plane->in_flight) > 3)
            {
                msg_Warn(vd, "%s discarded due to Wayland congestion", i == PLANE_VID ? "Video" : "Sub");
                spe_delete(&plane->spe_next);
                continue;
            }
