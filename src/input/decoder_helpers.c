...
int decoder_UpdateVideoOutput( decoder_t *dec, vlc_video_context *vctx_out )
{
    ...
    if (unlikely(dsc == NULL)) {
        msg_Warn(dec, "%s: chroma description for %.4s missing", __func__, (char*)&dec->fmt_out.i_codec);
    ...
    int rv;
    rv = dec->cbs->video.format_update( dec, vctx_out );
    if ( rv != 0 ) {
        msg_Warn(dec, "%s: format update failed", __func__);
    }
    return rv;
}
